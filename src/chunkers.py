        ...
        try:
            ...
            for doc in docs:
                if not isinstance(doc, Document):
                    logger.warning(f"Non-Document object received: {type(doc)}")
                    doc = Document(page_content=str(doc), metadata={"source": source})
                    ...
                    if current_length + sentence_length <= 200:
                        ...
                        if chunk_sentences:
                            ...
                            chunks.append(Document(
                                ...
                                metadata={"source": source, "page": doc.metadata.get("page", 0)}
                            ))
            ...
            logger.debug(f"Created {len(chunks)} chunks for {source}")
            logger.debug(f"Chunk content for {source}: {[chunk.page_content for chunk in chunks]}")
            return chunks
        except Exception as e:
            logger.error(f"Error building chunks for {source}: {str(e)}")
            return []
